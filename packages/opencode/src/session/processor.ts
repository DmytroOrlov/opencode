...
export type Result =
  ...
  | "stop"
  | "continue"
  | { type: "failover"; error: unknown; mode: "restart" | "continue" }
          ...
          yield* Effect.gen(function* () {
            ...
            Effect.retry(
              SessionRetry.policy({
                ...
                parse,
                failover: () => input.failoverAvailable === true,
                canRetry: () =>
                  ctx.executingToolCalls.size === 0 &&
                  ctx.settledExecutedToolCalls.size === 0 &&
                  !aborted &&
                  !ctx.blocked &&
                  !ctx.needsCompaction,
                set: (info) =>
            ...
            Effect.ensuring(
              Effect.gen(function* () {
                if (failover) {
                  if (failover.mode === "restart") yield* restart(baseline, messageBaseline, false)
                  return
                }
                if (continuation) {
                  return
