...
import { beginAttemptTelemetry } from "@/session/llm/attempt-telemetry"
...
test("attempt telemetry disposes an attached provider exactly once", () => {
  const reset: string[] = []
  const calls: string[] = []
  const lateCalls: string[] = []
  const lifecycle = beginAttemptTelemetry({ reset: () => reset.push("reset") })
  lifecycle.attachProvider({
  ...
  lifecycle.discard()
  lifecycle.attachProvider({
    finalize: () => lateCalls.push("finalize"),
    discard: () => lateCalls.push("discard"),
  })
  expect(reset).toEqual(["reset"])
  expect(calls).toEqual(["finalize"])
  expect(lateCalls).toEqual(["discard"])
})
...
test("teardown arbitration: pending provider sample beats an already-valid fallback", async () => {
  ...
  expect(attempt).toBeDefined()
  const lifecycle = beginAttemptTelemetry({ reset: () => {} })
  lifecycle.attachProvider(attempt!)
  ...
  lifecycle.attachFallback(fallback)
  lifecycle.finalize()
...
test("short generation: i0-only provider terminal lets the fallback freeze its rate", async () => {
  ...
  const lifecycle = beginAttemptTelemetry({ reset: () => {} })
  lifecycle.attachProvider(attempt!)
  lifecycle.attachFallback(fb.attempt)
  lifecycle.finalize()
  expect(a.holder.providerDecodeAccepted).toBeUndefined()
