...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("opencode.spawn")(function* (args: string[], opts?: SpawnOpts) {
      ...
      const command = ChildProcess.make("bun", ["run", cliEntry, "--tls-ca-mode=system", ...args], {
        cwd: home,
        env: { ...env, ...opts?.env },
        extendEnv: true,
        stdin: "ignore",
    ...
    const startRun = Effect.fn("opencode.startRun")(function* (message: string, opts?: RunOpts) {
      ...
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bun.spawn(["bun", "run", cliEntry, "--tls-ca-mode=system", ...runArgs(message, opts)], {
            cwd: home,
    ...
    const serve = Effect.fn("opencode.serve")(function* (opts?: ServeOpts) {
      ...
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bun.spawn(["bun", "run", cliEntry, "--tls-ca-mode=system", ...argv], {
            cwd: home,
